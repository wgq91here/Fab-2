...
class Profile extends CActiveRecord
{
	...
	public function rules()
	{
		...
		foreach ($model as $field) {
			...
			if ($field->field_type=='VARCHAR'||$field->field_type=='TEXT') {
				...
				if ($field->error_message) $field_rule['message'] = Yii::t("FabModule.fab", $field->error_message);
	...
	{
		$labels = array(
			'user_id' => Yii::t("FabModule.fab", 'User ID'),
			'profile_id' => Yii::t("FabModule.fab", 'Profile ID'),
		);
		$model=ProfileField::model()->forOwner()->findAll();
		
		foreach ($model as $field)
			$labels[$field->varname] = Yii::t("FabModule.fab", $field->title);
