...
class ExcelWriterXML_Sheet
{
    ...
    public function writeHtml($row, $column, $data, $style = null)
    {
        $this->writeData('Html', $row, $column, $data, $style);
    }
    ...
    public function getSheetXML()
    {
        ...
        foreach ($this->cells as $row => $rowData) {
            ...
            foreach ($rowData as $column => $cell) {
                ...
                if ($type === 'Html') {
                    $xml .= '				<ss:Data ss:Type="String" xmlns="http://www.w3.org/TR/REC-html40">';
                    $xml .= strip_tags(html_entity_decode(nl2br($data), ENT_QUOTES, 'UTF-8'), '<br><b><i><u><a><strong>');
                    $xml .= '</ss:Data>' . "\r";
                } else {
     ...
     */
    function cellWidth($row, $col, $width = 48)
    {
     ...
     */
    function columnWidth($col, $width = 48)
    {
     ...
     */
    function cellHeight($row, $col, $height = 12.5)
    {
     ...
     */
    function rowHeight($row, $height = 12.5)
    {
        $this->rowHeight[$row] = $height;
    }
