...
(function ($) {
    ...
    var arrFontsize = [
        {n: 'xx-small', wkn: 'x-small', s: '8pt', t: '極小'},
        {n: 'x-small', wkn: 'small', s: '10pt', t: '特小'},
        {n: 'small', wkn: 'medium', s: '12pt', t: '小'},
        {n: 'medium', wkn: 'large', s: '14pt', t: '中'},
        {n: 'large', wkn: 'x-large', s: '18pt', t: '大'},
        {n: 'x-large', wkn: 'xx-large', s: '24pt', t: '特大'},
        {n: 'xx-large', wkn: '-webkit-xxx-large', s: '36pt', t: '極大'}
    ];
    var menuAlign = [
        {s: '靠左對齊', v: 'justifyleft'},
        {s: '置中', v: 'justifycenter'},
        {s: '靠右對齊', v: 'justifyright'},
        {s: '左右對齊', v: 'justifyfull'}
    ], menuList = [
        {s: '數字列表', v: 'insertOrderedList'},
        {s: '符號列表', v: 'insertUnorderedList'}
    ];
    ...
    var htmlAbout = '<div style="width:200px;word-wrap:break-word;word-break:break-all;"><p><span style="font-size:20px;color:#1997DF;">xhEditor</span><br />v1.0.0 rc2 build 100401</p><p>xhEditor是一個基於jQuery開發的跨平台開源迷你XHTML編輯器組件。</p><p><a href="http://xheditor.com/" target="_blank">http://xheditor.com/</a></p></div>';
    ...
    var arrTools = {GStart: {}, GEnd: {}, Separator: {}, Cut: {t: '剪下 (Ctrl+X)'}, Copy: {t: '複製 (Ctrl+C)'}, Paste: {t: '貼上 (Ctrl+V)'}, Pastetext: {t: '貼上文本', h: isIE ? 0 : 1}, Blocktag: {t: '段落標籤', h: 1}, Fontface: {t: '字型', h: 1}, FontSize: {t: '字型大小', h: 1}, Bold: {t: '粗體 (Ctrl+B)', s: 'Ctrl+B'}, Italic: {t: '斜體 (Ctrl+I)', s: 'Ctrl+I'}, Underline: {t: '底線 (Ctrl+U)', s: 'Ctrl+U'}, Strikethrough: {t: '刪除線 (Ctrl+S)', s: 'Ctrl+S'}, FontColor: {t: '字型顏色', h: 1}, BackColor: {t: '背景顏色', h: 1}, Removeformat: {t: '刪除文字格式'}, Align: {t: '對齊', h: 1}, List: {t: '列表', h: 1}, Outdent: {t: '減少縮排 (Shift+Tab)', s: 'Shift+Tab'}, Indent: {t: '增加縮排 (Tab)', s: 'Tab'}, Link: {t: '超連結', h: 1}, Unlink: {t: '取消超連結'}, Img: {t: '圖片', h: 1}, Flash: {t: 'Flash動畫', h: 1}, Media: {t: '多媒體文件', h: 1}, Emot: {t: '表情', s: 'ctrl+e', h: 1}, Table: {t: '表格', h: 1}, Source: {t: '原始碼'}, Preview: {t: '預覽'}, Fullscreen: {t: '全螢幕編輯 (Esc)', s: 'Esc'}, About: {t: '關於 xhEditor', h: 1}};
    var toolsThemes = {
    ...
    $.xheditor = function (textarea, options) {
        ...
        this.init = function () {
            ...
            _jDoc.keydown(_this.checkShortcuts).keypress(_this.forcePtag).bind('mousedown click',function (ev) {
                _jText.trigger(ev.type);
            }).keyup(_this.triggerChange);
        ...
        this.saveUnload = function () {
            _this.getSource();
        };
        this.triggerChange = function () {
            _jText.trigger('change');
        }
        ...
        this.processHTML = function (sHtml, mode) {
            ...
            if (mode == 'write') {//write
                ...
                //保存屬性值:src,href
                function saveValue(all, tag, attr, n, q, v) {
                    return all.replace(attr, attr + ' _xhe_' + n + '="' + v + '"');
                }
                ...
                sHtml = sHtml.replace(/<img(?:\s+[^>]*?)?(\s+alt\s*=\s*(["']?)\s*(.*?)\s*\2)(?:\s+[^>]*?)?\/?>/ig, function (all, attr, q, alt) {
                    return alt ? all : all.replace(attr, '');
                });//寫瀏覽器前img強制去空alt以增加兼容性
            }
            else {//read
                //恢復屬性值src,href
                ...
                if (isSafari) {
                    ...
                    var arrAppleSpan = [
                        {r: /font-weight:\sbold/ig, t: 'strong'},
                ...
                sHtml = sHtml.replace(/<img(\s+[^>]+?)\/?>/ig, function (all, attr) {
                    if (!attr.match(/\s+alt\s*(["']?).*?\1(\s|$)/i))attr += ' alt=""';
                    return '<img' + attr + ' />';
                });//img強制加alt
            }
        ...
        this.formatXHTML = function (sHtml, bFormat) {
            ...
            while (last.length > 0) {
                if (!stack.last() || !specialTags[stack.last()]) {
                    ...
                    else if (last.substring(0, 2) == '</') {//結束標籤
            ...
            function addHtmlFrag(html, tagName, bStart) {
                if (tagName == 'pre' && !bStart)stopFormat = false;//恢復格式化
                ...
                if (bFormat == true && !stopFormat) {
                    ...
                    if (lastTag == 'pre' && lastTagStart)stopFormat = true;//禁止對pre內部的元素進行格式化
        ...
        this.showMenu = function (menuitems, callback) {
            ...
            $.each(menuitems, function (n, v) {
                jItem = $('<a href="javascript:void(0);" title="' + (v.t ? v.t : v.s) + '">' + v.s + '</a>').click(function () {
                    callback(v.v);
                    _this.hidePanel();
                    return false;
                }).mousedown(function () {
        ...
        this.showColor = function (callback) {
            ...
            $.each(itemColors, function (n, v) {
                c++;
                jItem = $('<a href="javascript:void(0);" title="' + v + '" style="background:' + v + '"></a>').click(function () {
                    callback(v);
                    _this.hidePanel();
                    return false;
                }).mousedown(function () {
        ...
        this.showEmot = function (group) {
            ...
            $('a', jList).click(function () {
                var jA = $(this);
                _this.pasteHTML('<img emot="' + jA.attr('emot') + '" alt="' + jA.attr('title') + '">');
                _this.hidePanel();
                return false;
            });
            jEmot.append(jList);
            ...
            if (gcount > 1) {
                ...
                $('a', jGroup).click(function () {
                    selEmotGroup = $(this).attr('group');
                    _this.exec('Emot');
                    return false;
                });
                jEmot.append(jGroup);
        ...
        this.uploadInit = function (jText, tourl, upext, setFunc) {
            ...
            if (tourl.substr(0, 1) == '!')//自定義上傳管理頁
            {
                jUpBtn.click(function () {
                    ...
                    _this.showIframeModal('上傳文件', tourl.substr(1), setUploadUrl, null, null, function () {
                        bShowPanel = true;
                    });
                });
            }
                ...
                jFile.change(function () {
                    ...
                    if (sFile != '') {
                        if (sFile.match(new RegExp('\.(' + upext.replace(/,/g, '|') + ')$', 'i'))) {
                            ...
                            _this.showModal('文件上傳中(Esc取消上傳)', '<div style="margin:22px 0;text-align:center;line-height:30px;">文件上傳中，請稍候……<br /><img src="' + skinPath + 'img/loading.gif"></div>', 320, 150, function () {
                                bShowPanel = true;
                                upload.remove();
                            });
                            upload.start();
        ...
        this.showModal = function (title, content, w, h, onRemove) {
            ...
            this.modalEscCheck = function (ev) {
                if (ev.which == 27) {
                    _this.removeModal();
                    return false;
                }
        ...
        this.showDialog = function (content) {
            ...
            if (jSave.length == 1) {
                jContent.find('input,select').keypress(function (ev) {
                    if (ev.which == 13) {
                        jSave.click();
                        return false;
                    }
                ...
                if (!_this.settings.clickCancelDialog) {
                    ...
                    var jFixCancel = $('<div class="xheFixCancel"></div>').appendTo('body').mousedown(function () {
                        return false;
                    });
        ...
        this.clickCancelPanel = function () {
            ...
            switch (cmd) {
                case 'cut':
                    try {
                        ...
                        alert('您的瀏覽器安全設置不允許使用剪下操作，請使用鍵盤快捷鍵(Ctrl + X)來完成');
                    }
                    ;
                    break;
                case 'copy':
                    try {
                        ...
                        alert('您的瀏覽器安全設置不允許使用複製操作，請使用鍵盤快捷鍵(Ctrl + C)來完成');
                    }
                    break;
                case 'paste':
                    try {
                        ...
                        alert('您的瀏覽器安全設置不允許使用貼上操作，請使用鍵盤快捷鍵(Ctrl + V)來完成');
                    }
                    break;
                case 'pastetext':
    ...
    $(function () {
        ...
        $.fn.val = function (value) {
            var editor;
            return (this[0] && (editor = this[0].editor)) ? (value === undefined ? editor.getSource() : editor.setSource(value)) : this.oldVal(value);
        }
