        ...
        this.uploadInit = function (jText, tourl, upext, setFunc) {
            ...
            if (tourl.substr(0, 1) == '!')//自定义上传管理页
            {
                jUpBtn.click(function () {
                    ...
                    _this.showIframeModal('上传文件', tourl.substr(1), setUploadUrl, null, null, function () {
                        bShowPanel = true;
                    });
                });
            }
                ...
                jFile.change(function () {
                    ...
                    if (sFile != '') {
                        if (sFile.match(new RegExp('\.(' + upext.replace(/,/g, '|') + ')$', 'i'))) {
                            ...
                            _this.showModal('文件上传中(Esc取消上传)', '<div style="margin:22px 0;text-align:center;line-height:30px;">文件上传中，请稍候……<br /><img src="' + skinPath + 'img/loading.gif"></div>', 320, 150, function () {
                                bShowPanel = true;
                                upload.remove();
                            });
                            upload.start();
