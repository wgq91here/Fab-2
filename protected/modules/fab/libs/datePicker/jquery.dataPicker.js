(function (D) {
    D.fn.extend({renderCalendar: function (P) {
        var X = function (Y) {
        ...
        if (P.showHeader != D.dpConst.SHOW_HEADER_NONE) {
            ...
            for (var S = Date.firstDayOfWeek; S < Date.firstDayOfWeek + 7; S++) {
                ...
                M.append(jQuery(X("th")).attr({scope: "col", abbr: R, title: R, "class": (H == 0 || H == 6 ? "weekend" : "weekday")}).html(P.showHeader == D.dpConst.SHOW_HEADER_SHORT ? R.substr(0, 1) : R))
            }
        }
        var E = D(X("table")).attr({cellspacing: 2, className: "jCalendar"}).append((P.showHeader != D.dpConst.SHOW_HEADER_NONE ? D(X("thead")).append(M) : X("thead")));
        var F = D(X("tbody"));
    ...
    D.extend(A.prototype, {init: function (E) {
        ...
        if (L.inline) {
            ...
            I = {top: K.top + L.verticalOffset, left: K.left + L.horizontalOffset};
            var J = function (Q) {
                var O = Q.target;
                var P = D("#dp-popup")[0];
                while (true) {
    ...
    D.dpText = {TEXT_PREV_YEAR: "Previous year", TEXT_PREV_MONTH: "Previous month", TEXT_NEXT_YEAR: "Next year", TEXT_NEXT_MONTH: "Next month", TEXT_CLOSE: "Close", TEXT_CHOOSE_DATE: "&nbsp;&nbsp;&nbsp;&nbsp;"};
